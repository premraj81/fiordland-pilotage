...
import { saveChecklist, updateChecklist, getChecklist, getChecklists } from '../lib/db';
...
function VesselNameInput({ value, onChange, knownShips = [] }: { value: string, onChange: (val: string) => void, knownShips: ShipData[] }) {
    ...
    // Use DB ships passed from parent
    const allShips = knownShips;
                        ...
                        <div className="absolute z-50 left-0 right-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-100 max-h-60 overflow-y-auto">
                            {suggestions.map((ship, idx) => (
                                ...
                                >
                                    ...
                                    <span className="text-xs text-gray-500">{ship.cruise_line}</span>
                                </button>
                            ))}
                        </div>
                    )}
                ...
                <div className="bg-sky-50 border border-sky-100 rounded-xl p-4 animate-in slide-in-from-top-2 fade-in duration-300">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        ...
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <span className="text-sm font-medium text-gray-900 truncate block" title={selectedShip.cruise_line}>{selectedShip.cruise_line}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span className="text-sm font-medium text-gray-900">{selectedShip.gross_tonnage}</span>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
                            <LogEntryTable entries={historyEntries} onEntriesChange={() => setHistoryEntries([])} />
                        </div>
                        <div className="bg-white px-6 py-4 border-t border-gray-100 flex justify-end shrink-0">
                            <button onClick={() => setIsHistoryOpen(false)} className="px-6 py-2 bg-gray-100 text-gray-700 font-bold rounded-lg hover:bg-gray-200 transition-colors">Close</button>
                        </div>
                    </div>
                </div>
            )}
...
export default function ChecklistForm() {
    ...
    const [checklistConfig, setChecklistConfig] = useState<any>(null);
    const [shipsList, setShipsList] = useState<ShipData[]>([]);
    ...
    useEffect(() => {
        const loadConfig = async () => {
            try {
                const [schemaRes, shipsRes] = await Promise.all([
                    fetch('/api/config/checklist_schema'),
                    fetch('/api/ships')
                ]);

                if (schemaRes.ok && shipsRes.ok) {
                    setChecklistConfig(await schemaRes.json());
                    setShipsList(await shipsRes.json());
                } else {
                    console.error("Failed to fetch config or ships");
                }
            } catch (e) {
    ...
    const handleAddLog = async () => {
        ...
        let ship = shipsList.find(s => s.name.toLowerCase() === vesselName.toLowerCase());
    ...
    if (loadingConfig) return <div className="min-h-screen flex items-center justify-center text-fiordland-800"><Loader className="w-8 h-8 animate-spin mr-2" /> Loading Configuration...</div>;
    if (!type || !checklistConfig || !checklistConfig[type]) return <div className="p-8 text-center text-red-500">Invalid Checklist Type.</div>;

    const schema = checklistConfig[type];
    ...
    const handleSaveProgress = async () => {
        ...
        try {
            ...
            if (id) {
                ...
                id = await saveChecklist({ type: type as any, createdAt: new Date(), data: dataToSave });
                setChecklistId(id);
            }
            ...
            sectionsToClear.forEach(secId => { if (nextFormData[secId]) nextFormData[secId] = {}; });
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            if (id) await updateChecklist(id, { data: dataToSave, pdfUrl: newPdfUrl });
            else await saveChecklist({ type: type as any, createdAt: new Date(), data: dataToSave, pdfUrl: newPdfUrl });
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8 pb-20">
            <button onClick={() => navigate('/')} className="flex items-center text-fiordland-500 hover:text-fiordland-900 transition-colors">
            ...
            <div className="space-y-2">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full md:w-auto self-start rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border font-medium text-fiordland-900" />
                </div>
                {schema.description && (
            ...
            <div className="space-y-8">
                {schema.sections.map((section: any) => (
                    <RenderSection
                        ...
                        setIsRouteReversed={setIsRouteReversed}
                        shipsList={shipsList}
                    />
                ))}
                ...
                <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        {!showTrainee && (
                            <button onClick={() => setShowTrainee(true)} className="flex items-center gap-2 text-sm font-medium text-brand-teal hover:text-teal-700 bg-teal-50 px-3 py-1.5 rounded-lg transition-colors border border-teal-100">
                                <UserPlus className="w-4 h-4" /> Add Trainee Pilot
                            </button>
                        )}
                    </div>
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <input type="text" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border" placeholder="Enter Master's Name" value={names.master || ''} onChange={(e) => setNames({ ...names, master: e.target.value })} />
                            <SignaturePad label="Master Signature" onChange={(data) => setSignatures((prev: any) => ({ ...prev, 'Master': data }))} />
                        </div>
                        <div className="space-y-4">
                            <label className="block text-sm font-medium text-gray-700">Pilot's Name</label>
                            <select className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border bg-white" value={names.pilot || ''} onChange={(e) => setNames({ ...names, pilot: e.target.value })}>
                                <option value="">Select Pilot...</option>
                                {PILOTS.map(p => <option key={p} value={p}>{p}</option>)}
                            </select>
                            <SignaturePad label="Pilot Signature" onChange={(data) => setSignatures((prev: any) => ({ ...prev, 'Pilot': data }))} />
                        </div>
                        {showTrainee && (
                            <div className="md:col-span-2 mt-8 pt-8 border-t border-gray-100 relative bg-gray-50 rounded-xl p-6">
                                <button onClick={() => setShowTrainee(false)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
                                <h3 className="text-lg font-semibold text-fiordland-800 mb-4 flex items-center gap-2"><UserPlus className="w-5 h-5 text-brand-teal" /> Trainee Pilot</h3>
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="space-y-1">
                                        <label className="block text-sm font-medium text-gray-700">Trainee's Name</label>
                                        <select className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border bg-white" value={names.trainee || ''} onChange={(e) => setNames({ ...names, trainee: e.target.value })}>
                                            <option value="">Select Trainee...</option>
                                            {TRAINEES.map(t => <option key={t} value={t}>{t}</option>)}
                                        </select>
                                    </div>
                                    <SignaturePad label="Trainee Signature" onChange={(data) => setSignatures((prev: any) => ({ ...prev, 'Trainee': data }))} />
                                </div>
                            </div>
                        )}
                    </div>
                ...
                {schema.finalSections && schema.finalSections.map((section: any) => (
                    <RenderSection key={section.id} section={section} formData={formData} handleInputChange={handleInputChange} isRouteReversed={isRouteReversed} setIsRouteReversed={setIsRouteReversed} shipsList={shipsList} />
                ))}
                    ...
                    <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                            {schema.finalSignatures.map((role: string) => (
                                <SignaturePad key={role} label={`${role}`} onChange={(data) => setSignatures((prev: any) => ({ ...prev, [role]: data }))} />
                            ))}
                        </div>
                    </section>
                )}
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 lg:static lg:bg-transparent lg:border-none lg:p-0 flex justify-end gap-3">
                <button onClick={() => setIsLogOpen(true)} className="bg-white text-fiordland-700 border border-fiordland-200 px-6 py-4 rounded-xl font-bold shadow-sm hover:bg-fiordland-50 transition-all flex items-center justify-center gap-2">
                    <BookOpen className="w-5 h-5" /> <span className="hidden sm:inline">Log Book</span>
                </button>
                <button onClick={handleSaveProgress} disabled={saving} className="w-full lg:w-auto bg-white text-brand-teal border border-brand-teal px-6 py-4 rounded-xl font-bold shadow-sm hover:bg-teal-50 transition-all flex items-center justify-center gap-2">
                    <Save className="w-5 h-5" /> Save Progress
                </button>
                <button onClick={handleComplete} disabled={saving} className="w-full lg:w-auto bg-brand-teal text-white px-8 py-4 rounded-xl font-bold shadow-lg shadow-teal-500/20 hover:bg-teal-600 transition-all flex items-center justify-center gap-2">
                    {saving ? 'Processing...' : <><CheckCircle className="w-5 h-5" /> Sign & Complete</>}
                </button>
            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="bg-fiordland-900 px-6 py-4 flex justify-between items-center">
                            <h3 className="text-white font-bold text-lg flex items-center gap-2"><BookOpen className="w-5 h-5 text-brand-teal" /> Add Log Entry</h3>
                            <button onClick={() => setIsLogOpen(false)} className="text-gray-400 hover:text-white transition-colors"><X className="w-6 h-6" /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div className="bg-blue-50 text-blue-800 p-3 rounded-lg text-sm font-medium">Vessel: {formData.header?.vesselName || "Unknown"}</div>
                            <div className="space-y-1">
                                <label className="text-sm font-medium text-gray-700">Log Note</label>
                                <textarea value={logContent} onChange={e => setLogContent(e.target.value)} className="w-full rounded-lg border-gray-300 focus:ring-brand-teal focus:border-brand-teal h-32 p-3" placeholder="Enter observation, incident, or note..." />
                            </div>
                        </div>
                        <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100">
                            <button onClick={() => setIsLogOpen(false)} className="px-4 py-2 text-gray-700 font-medium hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
                            <button onClick={handleAddLog} className="px-6 py-2 bg-brand-teal text-white font-bold rounded-lg hover:bg-teal-700 transition-colors shadow-sm flex items-center gap-2"><Save className="w-4 h-4" /> Save Entry</button>
                        </div>
                    </div>
                </div>
            )}
...
function RenderSection({ section, formData, handleInputChange, isRouteReversed, setIsRouteReversed, shipsList = [] }: any) {
    if (section.type === 'grading') {
        return (
            <section className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100"><h2 className="text-xl font-semibold text-fiordland-800">{section.title}</h2></div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {section.items.map((item: any) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="p-4 align-top"><input type="text" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-2 text-center uppercase" maxLength={3} value={formData[section.id]?.[`${item.id}_yesNo`] || ''} onChange={(e) => handleInputChange(section.id, `${item.id}_yesNo`, e.target.value)} /></td>
                                    <td className="p-4 align-top"><input type="number" min="1" max="5" className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-2 text-center font-bold" value={formData[section.id]?.[`${item.id}_grade`] || ''} onChange={(e) => handleInputChange(section.id, `${item.id}_grade`, e.target.value)} /></td>
                                    <td className="p-4 align-top"><textarea rows={2} className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-2 text-sm" placeholder="Enter comments..." value={formData[section.id]?.[`${item.id}_comment`] || ''} onChange={(e) => handleInputChange(section.id, `${item.id}_comment`, e.target.value)} /></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
    ...
    if (section.type === 'info') {
        return (
            <section className="bg-blue-50/50 rounded-2xl p-6 shadow-sm border border-blue-100">
                ...
                <div className="space-y-2 text-sm text-gray-700 pl-4 border-l-4 border-blue-200">{section.text.map((line: string, i: number) => <p key={i}>{line}</p>)}</div>
            </section>
        );
    }
                ...
                {section.fields?.map((field: any) => {
                    ...
                    if (field.id === 'vesselName') {
                        return (
                            <div key={field.id} className={`${widthClass} space-y-1 relative z-20`}>
                                ...
                                <VesselNameInput value={formData[section.id]?.[field.id] || ''} onChange={(val) => handleInputChange(section.id, field.id, val)} knownShips={shipsList} />
                            </div>
                        );
                    }
                    if (field.type === 'route-selector') {
                        const routes = isRouteReversed ? ['Dusky to Breaksea', 'Doubtful to Thompson', 'Milford', 'Stewart Island'] : ['Milford', 'Thompson to Doubtful', 'Breaksea to Dusky', 'Stewart Island'];
                            ...
                            <div key={field.id} className={`${widthClass}`}>
                                <div className="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-200">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button onClick={() => setIsRouteReversed(!isRouteReversed)} className="flex items-center gap-2 bg-teal-50 text-teal-700 hover:bg-teal-100 font-semibold px-4 py-2 rounded-lg transition-colors border border-teal-200 shadow-sm">
                                            <ArrowUpDown className="w-4 h-4" /> {isRouteReversed ? 'Switch to North-South' : 'Switch to South-North'}
                                        </button>
                                    </div>
                                    <div className="space-y-2">
                                        {routes.map((route) => (
                                            <label key={route} className="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:border-brand-teal transition-all shadow-sm">
                                                <input type="checkbox" checked={currentValues.includes(route)} onChange={(e) => { const newValues = e.target.checked ? [...currentValues, route] : currentValues.filter((v: string) => v !== route); handleInputChange(section.id, field.id, newValues); }} className="w-7 h-7 rounded text-brand-teal focus:ring-brand-teal border-gray-300" />
                    ...
                    return (
                        <div key={field.id} className={`${widthClass} space-y-1`}>
                            <label className="block text-sm font-medium text-gray-700">{field.label}</label>
                            {field.type === 'textarea' ? (
                                <textarea className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border h-32" onChange={(e) => handleInputChange(section.id, field.id, e.target.value)} placeholder="Enter notes here..." value={formData[section.id]?.[field.id] || ''} />
                            ) : (
                                <input type={field.type} className="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-teal focus:ring-brand-teal p-3 border" onChange={(e) => handleInputChange(section.id, field.id, e.target.value)} value={formData[section.id]?.[field.id] || ''} />
                            )}
                        </div>
                    );
                })}
                    ...
                    <div className="w-full px-2 mt-2">
                        {section.items.map((item: string, idx: number) => (
                            <label key={idx} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer group">
                                <input type="checkbox" checked={formData[section.id]?.[`item-${idx}`] || false} className="mt-0.5 w-7 h-7 rounded text-brand-teal focus:ring-brand-teal border-gray-300" onChange={(e) => handleInputChange(section.id, `item-${idx}`, e.target.checked)} />
